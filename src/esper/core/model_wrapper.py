...
class MorphableModel(nn.Module):
    ...
    async def load_kernel(self, layer_name: str, seed_idx: int, artifact_id: str) -> bool:
        ...
        if success:
            ...
            logger.warning(f"Failed to load kernel {artifact_id} into {layer_name}[{seed_idx}]")
...
def wrap(
    ...
    Args:
        ...
        target_layers: List of layer types to replace (default: [nn.Linear])
    ...
    if target_layers is None:
        target_layers = [nn.Linear]  # Default to Linear layers
        ...
        for name, child in module.named_children():
            ...
            if type(child) in target_layers:
                try:
                    ...
                    logger.info(f"Replaced {type(child).__name__} at '{child_name}' with KasminaLayer")
                except NotImplementedError as e:
                    logger.warning(f"Skipping {type(child).__name__} at '{child_name}': {e}")
...
def _create_kasmina_layer(
    ...
    if isinstance(original_layer, nn.Linear):
        ...
        # For Conv2d layers, use number of channels
    ...
    # Copy weights from original layer to preserve behavior
    if isinstance(original_layer, nn.Linear):
        with torch.no_grad():
        ...
        # For Conv2d, we need to adapt the weights to work with KasminaLayer's linear transform
        # This is a simplified approach - in practice, we might need more sophisticated handling
        with torch.no_grad():
            # Flatten the conv weights to work with linear layer
            original_weights = original_layer.weight.view(original_layer.out_channels, -1)
            # If the dimensions don't match our simplified Linear layer, we'll need to adapt
            if hasattr(kasmina_layer.default_transform, 'weight'):
                if original_weights.shape == kasmina_layer.default_transform.weight.shape:
                    kasmina_layer.default_transform.weight.copy_(original_weights)
                else:
                    # Use the first channel as a simplified approximation
                    kasmina_layer.default_transform.weight.copy_(
                        original_weights[:, :kasmina_layer.default_transform.weight.shape[1]]
                    )
            if original_layer.bias is not None and hasattr(kasmina_layer.default_transform, 'bias'):
                kasmina_layer.default_transform.bias.copy_(original_layer.bias)
