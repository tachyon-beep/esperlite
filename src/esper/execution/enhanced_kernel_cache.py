...
class KernelValidator:
    ...
    def validate_compatibility(
        ...
        if metadata.parameter_count > self.max_parameter_count:
            return False, f"Too many parameters: {metadata.parameter_count} > {self.max_parameter_count}"
        ...
        if not metadata.is_compatible_with_shape(target_shape_list):
            return False, f"Shape mismatch: kernel expects {metadata.input_shape}, got {target_shape_list}"
        ...
        if metadata.device_requirements and device_str not in metadata.device_requirements:
            return False, f"Device {device_str} not in requirements: {metadata.device_requirements}"
        
        # Check memory requirements
        max_allowed = max_memory_mb or self.max_memory_mb
        if metadata.memory_footprint_mb > max_allowed:
            return False, f"Memory footprint too large: {metadata.memory_footprint_mb}MB > {max_allowed}MB"
...
class EnhancedKernelCache(KernelCache):
        ...
        async with self._lock:
            ...
            if artifact_id in self.metadata_cache:
                ...
                if not is_valid:
                    ...
                    logger.warning(f"Kernel {artifact_id} compatibility failed: {error_msg}")
                    return None
            ...
            self._misses += 1
            logger.debug(f"Enhanced cache miss for kernel {artifact_id}, fetching with metadata")
            ...
            if not is_valid:
                self.compatibility_failures += 1
                logger.warning(f"Fetched kernel {artifact_id} compatibility failed: {error_msg}")
                return None
            
            # Estimate memory usage
            estimated_memory = self.validator.estimate_execution_memory(metadata, batch_size)
    ...
    def get_enhanced_stats(self) -> Dict[str, Any]:
        ...
        avg_kernel_params = (
            sum(info.get("parameter_count", 0) for info in self._cache_info.values()) /
        ...
        enhanced_stats = {
            **base_stats,
            ...
            "kernels_with_metadata": len([
                k for k in self._cache_info.keys() 
                if k in self.metadata_cache
            ])
        }
