...
class ErrorTracker:
    ...
    def record_error(self, error_context: ErrorContext):
        ...
        if len(self.error_history) > self.window_size:
            ...
            self.component_errors[old_error.component] = [
                e
                for e in self.component_errors[old_error.component]
            ...
            if old_error.kernel_id:
                self.kernel_errors[old_error.kernel_id] = [
                    e
                    for e in self.kernel_errors[old_error.kernel_id]
                    if e.timestamp > old_error.timestamp
                ]
    ...
    def get_error_rate(
        self, error_type: Optional[ErrorType] = None, time_window: float = 300.0
    ) -> float:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "problematic_kernels": [
                kernel_id
                for kernel_id in self.kernel_errors.keys()
...
class ErrorRecoveryManager:
    ...
    async def handle_error(
        ...
        try:
            ...
            strategy = self.recovery_strategies.get(
                error_context.error_type, RecoveryStrategy.FALLBACK
            )
            ...
            success = await self._execute_recovery(
                error_context, strategy, fallback_action
            )
    ...
    def _should_escalate(self, error_context: ErrorContext) -> bool:
        ...
        if (
            self.error_tracker.get_error_rate(error_context.error_type) > 0.1
        ):  # 0.1 errors/sec
            return True
    ...
    async def _execute_recovery(
        ...
        logger.info(
            f"Executing recovery strategy: {strategy.value} for {error_context.component}"
        )
    ...
    async def _retry_recovery(self, error_context: ErrorContext) -> bool:
        ...
        for attempt in range(max_retries):
            if attempt > 0:
                ...
                logger.info(
                    f"Retrying {error_context.component} in {delay}s (attempt {attempt + 1}/{max_retries})"
                )
            ...
            try:
                ...
                logger.info(
                    f"Retry attempt {attempt + 1} for {error_context.component}"
                )
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        success_rate = sum(1 for r in recent_recoveries if r["success"]) / max(
            len(recent_recoveries), 1
        )
...
class HealthMonitor:
    ...
    async def _check_system_health(self):
        ...
        # Check error rates
        error_rate = self.recovery_manager.error_tracker.get_error_rate(
            time_window=300.0
        )
        ...
        if torch.cuda.is_available():
            ...
            if memory_usage > self.health_thresholds["memory_usage"]:
                logger.warning("High memory usage detected: %.2%%", memory_usage)
