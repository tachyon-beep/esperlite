...
class KernelCache:
    ...
    def __init__(self, max_cache_size_mb: int = 512, max_entries: int = 128):
        ...
        logger.info(f"Initialized KernelCache with {max_cache_size_mb}MB max size, {max_entries} max entries")
    ...
    def _add_to_cache(self, artifact_id: str, kernel_tensor: torch.Tensor) -> None:
        ...
        while (
            (len(self._cache) >= self.max_entries or 
             self.total_size_mb + tensor_size_mb > self.max_cache_size_mb) and
            len(self._cache) > 0
        ):
            self._evict_lru()
