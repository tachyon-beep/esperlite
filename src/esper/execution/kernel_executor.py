...
class KernelValidator:
    ...
    def validate_module(self, module: nn.Module) -> Tuple[bool, str]:
        ...
        try:
            ...
            if param_count > self.max_parameters:
                return False, f"Module has too many parameters: {param_count} > {self.max_parameters}"
    ...
    def validate_shapes(
        ...
        if len(input_shape) != len(expected_input_shape):
            return False, f"Input rank mismatch: {len(input_shape)} vs {len(expected_input_shape)}"
        
        # Check all dimensions except batch (index 0)
        for i in range(1, len(input_shape)):
            if input_shape[i] != expected_input_shape[i]:
                return False, f"Input shape mismatch at dim {i}: {input_shape[i]} vs {expected_input_shape[i]}"
        
        if len(output_shape) != len(expected_output_shape):
            return False, f"Output rank mismatch: {len(output_shape)} vs {len(expected_output_shape)}"
        
        for i in range(1, len(output_shape)):
            if output_shape[i] != expected_output_shape[i]:
                return False, f"Output shape mismatch at dim {i}: {output_shape[i]} vs {expected_output_shape[i]}"
...
class RealKernelExecutor:
    ...
    def _deserialize_pickle(self, kernel_artifact: bytes) -> nn.Module:
        ...
        try:
            ...
            if not isinstance(module, nn.Module):
                raise KernelDeserializationError(f"Expected nn.Module, got {type(module)}")
    ...
    async def _execute_with_timeout(
        ...
        try:
            ...
            raise KernelExecutionError(f"Kernel execution timed out after {self.execution_timeout}s")
    ...
    def _validate_output_shape(
        ...
        if torch.any(torch.isnan(output_tensor)) or torch.any(torch.isinf(output_tensor)):
    ...
    def _apply_alpha_blending(
        ...
        if input_tensor.shape != kernel_output.shape:
            ...
            logger.debug(f"Shape mismatch for blending: {input_tensor.shape} vs {kernel_output.shape}")
    ...
    def _evict_lru(self):
        ...
        lru_kernel_id = min(self.cache_access_times.keys(), 
