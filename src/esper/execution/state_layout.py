...
class KasminaStateLayout:
    ...
    def __init__(self, num_seeds: int, device: torch.device):
        ...
        self.lifecycle_states = torch.zeros(
            num_seeds, dtype=torch.uint8, device=device
        )
        self.active_kernel_id = torch.zeros(
            num_seeds, dtype=torch.int64, device=device
        )
        self.alpha_blend = torch.zeros(
            num_seeds, dtype=torch.float16, device=device
        )
        ...
        self.exec_latency_us = torch.zeros(
            num_seeds, dtype=torch.uint16, device=device
        )
        self.error_count = torch.zeros(
            num_seeds, dtype=torch.int16, device=device
        )
        self.fallback_active = torch.zeros(
            num_seeds, dtype=torch.bool, device=device
        )
        
        logger.debug(f"Initialized KasminaStateLayout with {num_seeds} seeds on {device}")
    ...
    def transition_seed_state(
        ...
        logger.debug(f"Seed {seed_idx} transitioned from {old_state.name} to {new_state.name}")
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        return {
            ...
            "error_recovery_seeds": int((states == SeedLifecycleState.ERROR_RECOVERY).sum()),
