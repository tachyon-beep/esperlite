...
class HealthSignalBuffer:
    ...
    async def add_with_priority(self, signal: HealthSignal, priority: float):
        ...
        async with self._lock:
            if priority > 0.7:  # High priority
                ...
                if (
                    len(self.priority_signals) > self.max_size // 10
    ...
    async def get_signals_for_layer(
        self, layer_id: int, count: int = 100
    ) -> List[HealthSignal]:
        """Get recent signals for specific layer."""
        async with self._lock:
            layer_signals = [
                s for s in list(self.signals)[-count * 2 :] if s.layer_id == layer_id
            ]
...
class ProductionHealthCollector:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ProductionHealthCollector with buffer_size={buffer_size}"
        )
    ...
    def _compute_health_score(self, metrics: Dict[str, Any]) -> float:
        ...
        # Error rate impact
        error_rate = metrics.get("error_count", 0) / max(
