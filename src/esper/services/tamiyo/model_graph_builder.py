...
class ModelTopology:
    ...
    def get_layer_info(self, layer_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "connections": [
                target for source, target in self.connections if source == layer_name
            ],
        }
...
class LayerFeatureExtractor:
    ...
    def extract_features(
        ...
        if layer_signals:
            ...
            variance_activation = np.mean(
                [s.activation_variance for s in layer_signals]
            )
        ...
        type_encoding = self.layer_type_encoding.get(
            layer_type, self.layer_type_encoding["unknown"]
        )
        ...
        features.extend([in_degree, out_degree, in_degree + out_degree])  # Total degree
...
class ConnectionFeatureExtractor:
    ...
    def extract_edge_features(
        self, source_layer: str, target_layer: str, topology: ModelTopology
    ) -> List[float]:
...
class PerformanceFeatureExtractor:
    ...
    def extract_temporal_features(
        self, health_signals: List[HealthSignal], window_size: int = 100
    ) -> Dict[str, Any]:
...
class ModelGraphBuilder:
    ...
    def _extract_node_features(
        self, health_signals: List[HealthSignal], topology: ModelTopology
    ) -> torch.Tensor:
        ...
        for layer_name in topology.layer_names:
            ...
            if len(layer_features) < self.node_feature_dim:
                layer_features.extend(
                    [0.0] * (self.node_feature_dim - len(layer_features))
                )
    ...
    def _infer_topology_from_signals(
        ...
        topology = ModelTopology(
            ...
            connections=[
                (layer_names[i], layer_names[i + 1])
                for i in range(len(layer_names) - 1)
            ],
            parameter_counts={
                name: 1000 for name in layer_names
            },  # Default param count
        )
    ...
    def _compute_health_trends(
        ...
        for layer_name in topology.layer_names:
            ...
            if layer_signals:
                ...
                recent_scores = [
                    s.health_score for s in sorted_signals[-20:]
                ]  # Last 20 signals
    ...
    def create_test_topology(self, num_layers: int = 5) -> ModelTopology:
        ...
        layer_shapes = {f"layer_{i}": (64, 64) for i in range(num_layers)}

        connections = [(f"layer_{i}", f"layer_{i+1}") for i in range(num_layers - 1)]

        parameter_counts = {f"layer_{i}": 1000 * (i + 1) for i in range(num_layers)}
    ...
    def get_graph_statistics(self, graph_state: ModelGraphState) -> Dict[str, Any]:
        ...
        return {
            ...
            "edge_feature_dim": (
                graph_state.graph_data.edge_attr.size(1)
                if graph_state.graph_data.edge_attr is not None
                else 0
            ),
