...
class TamiyoTrainer:
    ...
    def _prepare_batch(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        node_features = torch.randn(batch_size * 4, 64, device=self.device)  # 4 nodes per graph
        ...
        for i in range(batch_size):
            ...
            edges = torch.tensor([[base_idx, base_idx+1, base_idx+1, base_idx+2, base_idx+2, base_idx+3],
                                 [base_idx+1, base_idx, base_idx+2, base_idx+1, base_idx+3, base_idx+2]], 
                                device=self.device)
        ...
        actions = torch.tensor([
            1.0 if exp.get("action") else 0.0 for exp in batch
        ], device=self.device)
        
        rewards = torch.tensor([
            exp.get("reward", 0.0) for exp in batch
        ], device=self.device)
    ...
    def _compute_ppo_loss(self, batch_tensors: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    def _compute_policy_loss(
        ...
        action_log_probs = torch.log(log_probs + 1e-8) * actions + torch.log(1 - log_probs + 1e-8) * (1 - actions)
    ...
    def _compute_final_metrics(self, training_metrics: List[Dict[str, float]]) -> Dict[str, float]:
