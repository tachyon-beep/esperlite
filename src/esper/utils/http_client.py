...
class AsyncHttpClient:
    ...
    def __init__(
        ...
        logger.debug(f"Initialized AsyncHttpClient with {max_connections} max connections")
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                async with self.session.request(method, url, **kwargs) as response:
                    ...
                    logger.debug(f"{method} {url} -> {response.status} (attempt {attempt + 1})")
                ...
                if attempt < self.max_retries:
                    ...
                    logger.error(f"{method} {url} failed after {self.max_retries + 1} attempts: {e}")
                    raise
