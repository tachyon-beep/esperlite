...
class MixedArchitectureModel(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256, num_classes: int = 10):
        ...
        self.proj = nn.Linear(128 * 8 * 8, embed_dim)  # Assuming 32x32 input -> 8x8 after pooling
...
class TestTransformerArchitectures:
    ...
    def test_transformer_block_wrapping(self):
        ...
        assert len(layer_names) >= 5  # At least attention + 2 norms + 2 linear layers in FFN
        ...
        linear_layers = [name for name in layer_names if 'ffn' in name and ('0' in name or '3' in name)]
        
        assert len(attention_layers) >= 1, f"Expected attention layers, got: {layer_names}"
        assert len(norm_layers) >= 2, f"Expected 2+ norm layers, got: {norm_layers}"
        assert len(linear_layers) >= 2, f"Expected 2+ linear layers, got: {linear_layers}"
    ...
    def test_transformer_forward_pass(self):
        ...
        assert max_diff < 10.0, f"Max difference {max_diff:.3f} suggests major implementation issue"
        
        print(f"Transformer forward pass: max_diff={max_diff:.3f} (acceptable for morphogenetic wrapper)")
...
class TestResNetArchitectures:
    ...
    def test_resnet_block_wrapping(self):
        ...
        assert len(conv_layers) >= 2, f"Expected 2+ conv layers, got: {conv_layers}"  # conv1, conv2, (optional shortcut conv)
        assert len(bn_layers) >= 2, f"Expected 2+ bn layers, got: {bn_layers}"      # bn1, bn2, (optional shortcut bn)
...
class TestMixedArchitectures:
    ...
    def test_mixed_architecture_wrapping(self):
        ...
        conv_layers = [name for name in layer_names if 'conv' in name]
        bn_layers = [name for name in layer_names if 'bn' in name or any('BatchNorm' in str(type(layer)) for layer in morphable_model.kasmina_layers.values())]
        linear_layers = [name for name in layer_names if 'linear' in name or 'proj' in name or 'classifier' in name]
        attention_layers = [name for name in layer_names if 'attn' in name]
        norm_layers = [name for name in layer_names if 'norm' in name and 'batch' not in name]
        ...
        assert len(linear_layers) >= 2, f"Expected 2+ linear layers, got: {linear_layers}"
        assert len(attention_layers) >= 1, f"Expected 1+ attention layers, got: {attention_layers}"
    ...
    def test_mixed_architecture_forward_pass(self):
        ...
        print(f"Mixed architecture forward pass: max_diff={max_diff:.3f} (acceptable for morphogenetic wrapper)")
...
class TestArchitectureDetection:
    ...
    def test_unsupported_layer_handling(self):
        ...
        class UnsupportedModel(nn.Module):
            def __init__(self):
                ...
                self.unsupported = nn.LSTM(5, 3, batch_first=True)  # LSTM not yet supported
        ...
        layer_names = morphable_model.get_layer_names()
        linear_layers = [name for name in layer_names if 'supported' in name or 'final' in name]
        lstm_layers = [name for name in layer_names if 'unsupported' in name]
        
        assert len(linear_layers) == 2, f"Expected 2 linear layers, got: {linear_layers}"
        assert len(lstm_layers) == 0, f"Expected 0 LSTM layers wrapped, got: {lstm_layers}"
    ...
    def test_selective_layer_wrapping(self):
        ...
        for name in layer_names:
            ...
            assert hasattr(layer, 'default_transform'), f"Layer {name} should be a KasminaLayer"
