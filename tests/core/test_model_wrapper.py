...
from esper.core.model_wrapper import wrap, MorphableModel, unwrap, _create_kasmina_layer
...
class TestMorphableModel:
    ...
    async def test_unload_kernel_success(self):
        ...
        mock_state_layout.get_active_seeds = Mock(return_value=torch.tensor([False, False, False, False]))
        ...
        assert success == True
        assert morphable.morphogenetic_active == False  # Should be False after unloading
    ...
    def test_get_model_stats(self):
        ...
        mock_state_layout1.get_active_seeds = Mock(return_value=torch.tensor([True, False, True, False]))
        ...
        mock_state_layout2.get_active_seeds = Mock(return_value=torch.tensor([False, True, False, False]))
...
class TestWrapFunction:
    ...
    def test_wrap_weights_copied(self):
        ...
        assert torch.allclose(original_linear1.weight, kasmina_layer1.default_transform.weight)
        assert torch.allclose(original_linear1.bias, kasmina_layer1.default_transform.bias)
...
class TestCreateKasminaLayer:
    ...
    def test_create_from_linear(self):
        ...
        # Check that weights are copied
        assert torch.allclose(linear_layer.weight, kasmina_layer.default_transform.weight)
