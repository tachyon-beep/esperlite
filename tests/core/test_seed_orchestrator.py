...
# Removed unused imports from test_infrastructure


class MockTestModel(nn.Module):
...
class TestSeedOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_analyze_seed_performance(
        ...
        active_seeds_mask = torch.tensor([True, True, False, False])
        layer.state_layout.get_active_seeds = Mock(return_value=active_seeds_mask)
    ...
    async def test_execute_modification_plan(self, orchestrator, test_model):
        ...
        plan = SeedModificationPlan(
            ...
            seed_modifications={
                0: {
                    ...
                    "initial_blend": 0.3,
                    "reasoning": "Load diverse kernel",
                },
                1: {
                    "action": "adjust_blend",
                    "new_blend": 0.5,
                    "reasoning": "Adjust blend factor",
                },
                2: {
                    "action": "unload_kernel",
                    "reasoning": "Unload underperforming kernel",
                },
            },
        ...
        success = await orchestrator._execute_modification_plan(
            layer, plan, model_state
        )

        assert success == True
        # Check that methods were called
        assert (
            orchestrator.integration_orchestrator.execute_adaptation_pipeline.call_count
            == 1
        )
    ...
    @pytest.mark.asyncio
    async def test_apply_architecture_modification_non_kasmina_layer(
