...
class TestKernelCache:
    ...
    async def test_lru_eviction(self, mock_fetch):
        ...
        mock_fetch.side_effect = [torch.randn(1024), torch.randn(1024), torch.randn(1024)]
    ...
    def test_remove_kernel(self):
        ...
        cache._cache_info["test-kernel"] = {"size_mb": 0.1, "added_at": 0, "last_accessed": 0}
        cache.total_size_mb = 0.1
    ...
    async def test_concurrent_access(self, mock_fetch):
        ...
        tasks = [
            cache.load_kernel("shared-kernel")
            for _ in range(5)
        ]
    ...
    def test_cache_size_calculation_with_real_tensors(self):
        ...
        cache._cache_info["small"] = {"size_mb": small_size_mb, "added_at": 0, "last_accessed": 0}
        cache._cache_info["medium"] = {"size_mb": medium_size_mb, "added_at": 1, "last_accessed": 1}
        cache._cache_info["large"] = {"size_mb": large_size_mb, "added_at": 2, "last_accessed": 2}
        cache.total_size_mb = small_size_mb + medium_size_mb + large_size_mb
        
        # Test size calculation
        stats = cache.get_cache_stats()
        assert stats["entries"] == 3
        assert abs(stats["total_size_mb"] - (small_size_mb + medium_size_mb + large_size_mb)) < 0.001
