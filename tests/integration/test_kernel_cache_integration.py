...
class TestKernelCacheUrzaIntegration:
    ...
    async def test_successful_kernel_fetch_from_urza(self, mock_client_class):
        ...
        async def mock_json():
            return {"kernel_binary_ref": "https://s3.example.com/kernels/test-kernel-123.bin"}
    ...
    async def test_concurrent_urza_requests(self, mock_client_class):
        ...
        def create_mock_responses(kernel_id):
            ...
            async def mock_json():
                return {"kernel_binary_ref": f"https://s3.example.com/kernels/{kernel_id}.bin"}
    ...
    @patch('os.getenv')
    @patch('esper.utils.http_client.AsyncHttpClient')
    async def test_custom_urza_url_from_environment(self, mock_client_class, mock_getenv):
        ...
        mock_getenv.assert_called_with("URZA_API_URL", "http://localhost:8000")
