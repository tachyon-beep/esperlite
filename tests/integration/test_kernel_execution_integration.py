...
class TestKernelExecutionIntegration:
    ...
    async def test_kernel_loading_and_execution(self):
        ...
        with patch.object(self.layer.kernel_cache, 'load_kernel_with_validation') as mock_load, \
    ...
    async def test_forward_pass_with_kernels(self):
        ...
        with patch.object(self.layer.kernel_cache, 'load_kernel_with_validation') as mock_load, \
            ...
            # Load kernel
            await self.layer.load_kernel(seed_idx=0, artifact_id="execution_test_kernel")
    ...
    async def test_multiple_seeds_execution(self):
        ...
        with patch.object(self.layer.kernel_cache, 'load_kernel_with_validation', side_effect=mock_load_side_effect), \
             patch.object(self.layer.kernel_cache, 'get_kernel_bytes', side_effect=mock_get_bytes_side_effect):
            
            # Load kernels into multiple seeds
            success1 = await self.layer.load_kernel(seed_idx=0, artifact_id="multi_kernel_0")
            success2 = await self.layer.load_kernel(seed_idx=1, artifact_id="multi_kernel_1")
    ...
    async def test_kernel_compatibility_filtering(self):
        ...
        with patch.object(self.layer.kernel_cache, 'load_kernel_with_validation') as mock_load:
            ...
            assert call_args[1]["target_shape"][1] == self.input_size  # Batch dim excluded
...
class TestKernelExecutionPerformance:
    ...
    async def test_kernel_loading_performance(self):
        ...
        with patch.object(self.layer.kernel_cache, 'load_kernel_with_validation') as mock_load, \
            ...
            assert avg_load_time < 100.0, f"Average load time {avg_load_time:.2f}ms exceeds target"
    ...
    def test_memory_usage(self):
        ...
        if torch.cuda.is_available():
            ...
            assert memory_increase_mb < 500.0, f"Memory increase {memory_increase_mb:.1f}MB exceeds target"
...
class TestSystemResilience:
    ...
    async def test_graceful_degradation(self):
        ...
        with patch.object(self.layer.kernel_cache, 'load_kernel_with_validation') as mock_load:
            ...
            success = await self.layer.load_kernel(seed_idx=0, artifact_id="failing_kernel")
            assert not success
    ...
    async def test_error_recovery_effectiveness(self):
        ...
        for i in range(9):
            try:
                ...
                from src.esper.execution.error_recovery import classify_kernel_error, create_error_context
