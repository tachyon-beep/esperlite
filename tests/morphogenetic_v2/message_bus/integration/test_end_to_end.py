...
class TestRealRedisIntegration:
    ...
    async def test_redis_command_acknowledgment(self, redis_test_client,
                                              message_bus_config,
                                              test_layer_registry):
        """Test command handling via Redis.
        
        Note: Acknowledgment publishing is not currently implemented,
        so we just verify command execution succeeded.
        """
        # Create client
        ...
        # Verify command was handled successfully
        assert result.success is True
        assert result.details.get("new_state") == "GERMINATED"
