...
class TestRedisStreamClient:
    ...
    async def test_disconnect(self):
        ...
        mock_redis = AsyncMock()
        mock_redis.aclose = AsyncMock()  # Redis uses aclose, not close
        client.redis = mock_redis
        client._connected = True

        # Add mock tasks - create real asyncio tasks
        async def dummy_coro():
            await asyncio.sleep(100)  # Long sleep so it doesn't finish
        
        task = asyncio.create_task(dummy_coro())
        client._background_tasks = [task]
        client._subscriptions = {"sub1": task}
    ...
    async def test_unsubscribe(self):
        ...
        task = asyncio.create_task(dummy_coro())
        client._subscriptions[sub_id] = task
    ...
    def test_serialize_message_with_compression(self):
        ...
        config = MessageBusConfig(
            ...
            compression_threshold=200  # Increased threshold to ensure small message isn't compressed
        )
        client = RedisStreamClient(config)
    ...
    def test_deserialize_message(self):
        ...
        # Uncompressed - use a real registered message type
        data = json.dumps({
