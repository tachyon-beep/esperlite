...
from src.esper.morphogenetic_v2.message_bus.schemas import (
    LayerHealthReport, SeedMetricsSnapshot, PerformanceAlert,
    StateTransitionEvent, AlertType, AlertSeverity
)
...
class TestTelemetryPublisher:
    ...
    async def test_batch_window(self, mock_client):
        ...
        # Start batch publisher task
        batch_task = asyncio.create_task(publisher._batch_publisher())
    ...
    def test_is_anomaly(self, mock_client, telemetry_config):
        ...
        # Edge case: zero std
        history = deque([1.0] * 20, maxlen=100)
        assert publisher._is_anomaly(history, 2.0)
