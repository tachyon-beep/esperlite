...
from src.esper.morphogenetic_v2.message_bus.utils import (
    CircuitState, CircuitBreakerConfig, CircuitBreaker, CircuitOpenError,
    MessageDeduplicator, RateLimiterConfig, RateLimiter,
    MessageBatcher, RetryPolicy, MetricsCollector
)
...
class TestMessageBatcher:
    ...
    async def test_batch_time_trigger(self, batcher):
        ...
        assert result is not None
        assert len(result) == 0  # Previous batch was empty
