...
from esper.morphogenetic_v2.common.feature_flags import FeatureFlagManager
...
class TestHybridKasminaLayer:
    ...
    def test_initialization(self, base_layer):
        ...
        assert layer.layer_id == "test_init"
        assert layer.force_implementation is None
        assert isinstance(layer.feature_flags, FeatureFlagManager)
        
        # Check implementation availability
        # At least one should be available
        assert layer.legacy_available or layer.chunked_available
        
        # Check metrics initialization
        assert layer.implementation_calls == {"legacy": 0, "chunked": 0}
        assert layer.implementation_latency == {"legacy": 0.0, "chunked": 0.0}
