...
class TestStateTensor:
    ...
    def test_initialization(self):
        """Test StateTensor initialization."""
        num_seeds = 50
        st = StateTensor(num_seeds)
        
        # Check dimensions
        assert st.num_seeds == num_seeds
        assert st.state_tensor.shape == (num_seeds, 4)
        assert st.health_scores.shape == (num_seeds,)
        assert st.error_counts.shape == (num_seeds,)
        assert st.alpha_blend.shape == (num_seeds,)
        
        # Check initial values
        assert (st.state_tensor == 0).all()  # All states start at DORMANT
        assert (st.health_scores == 1.0).all()
        assert (st.error_counts == 0).all()
        assert (st.alpha_blend == 0.0).all()
    ...
    def test_health_score_updates(self, state_tensor):
        ...
        state_tensor.update_health_scores(seed_ids, health_scores)
    ...
    def test_alpha_blend_updates(self, state_tensor):
        ...
        state_tensor.update_alpha_blend(seed_ids, alphas)
