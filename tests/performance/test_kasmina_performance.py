...
class TestKasminaPerformance:
    ...
    def test_dormant_seed_overhead(self, performance_config):
        ...
        max_overhead = max(performance_config["max_overhead_percent"] * 2, 15.0)  # More realistic threshold
        
        assert overhead < max_overhead, f"Overhead {overhead:.2f}% exceeds {max_overhead}% target"
    ...
    def test_conv2d_performance_overhead(self, performance_config):
        ...
        overhead = (kasmina_time - baseline_time) / baseline_time * 100
        max_overhead = min(performance_config["max_overhead_percent"] * 3, 20.0)  # Allow more overhead for Conv2d
        
        assert overhead < max_overhead, f"Conv2d overhead {overhead:.2f}% exceeds {max_overhead}% target"
    ...
    def test_transformer_performance_overhead(self, performance_config):
        ...
        assert overhead < max_overhead, f"Transformer overhead {overhead:.2f}% exceeds {max_overhead}% target"
    ...
    def test_layer_functionality_with_different_sizes(self, performance_config):
        ...
        for input_size, output_size in layer_configs:
            ...
            assert torch.isfinite(output).all(), \
                f"Layer {input_size}->{output_size} produced non-finite values"
        ...
        print(f"Successfully tested {len(layer_configs)} different layer configurations")
    ...
    def test_memory_usage_scaling(self):
        ...
        for input_size, output_size, num_seeds in layer_configs:
            ...
            state_memory = layer.state_layout.alpha_blend.numel() * layer.state_layout.alpha_blend.element_size()
            total_memory = param_memory + state_memory
            
            memory_usage.append(total_memory)
            
            print(f"Layer {input_size}→{output_size} (seeds={num_seeds}): {total_memory / 1024 / 1024:.2f} MB")
...
class TestModelPerformance:
    ...
    def test_full_model_overhead_linear(self, simple_linear_model, performance_config):
        ...
        assert overhead < max_overhead, f"Full model overhead {overhead:.2f}% exceeds {max_overhead}% target"
    ...
    def test_full_model_overhead_conv(self, simple_conv_model, performance_config):
        ...
        assert overhead < max_overhead, f"Full Conv model overhead {overhead:.2f}% exceeds {max_overhead}% target"
...
class TestLatencyBenchmarks:
    ...
    def test_single_forward_pass_latency(self, performance_config):
        ...
        assert p99_latency < max_acceptable_latency * 2, \
    ...
    def test_kernel_loading_latency(self, performance_config):
        ...
        print(f"Kernel loading latency (ms):")
        print(f"  Average: {avg_loading_time:.3f}")
        print(f"  Maximum: {max_loading_time:.3f}")
        
        # Loading should be fast for real-time adaptation
        assert avg_loading_time < 1.0, f"Average kernel loading {avg_loading_time:.3f}ms too slow"
        assert max_loading_time < 5.0, f"Maximum kernel loading {max_loading_time:.3f}ms too slow"