    ...
    def create_unhealthy_system_scenario() -> Dict[str, Any]:
        ...
        return {
            ...
            'health_signals': [
                ...
                HealthSignal(
                    layer_id=0, seed_id=0, chunk_id=0, epoch=100,
                ...
                HealthSignal(
                    layer_id=1, seed_id=0, chunk_id=0, epoch=100,
                ...
                *[
                    HealthSignal(
                        layer_id=i, seed_id=0, chunk_id=0, epoch=100,
...
class TestAutonomousServiceCore:
    ...
    async def test_end_to_end_decision_cycle_unhealthy_system(
        ...
        if decision is not None:
            ...
            assert isinstance(decision, AdaptationDecision), "Decision should be AdaptationDecision type"
            assert 0.0 <= decision.confidence <= 1.0, f"Confidence {decision.confidence} not in [0,1]"
            assert 0.0 <= decision.urgency <= 1.0, f"Urgency {decision.urgency} not in [0,1]"
            ...
            logger.info(f"Decision made: layer={decision.layer_name}, "
            ...
            logger.info("No decision made (acceptable for untrained model or stable system)")
        
        # Validate performance requirements
        assert decision_latency_ms < 100, f"Decision latency {decision_latency_ms}ms exceeds 100ms SLA"
    ...
    async def test_stable_system_no_intervention(
        ...
        assert decisions_made == 0, f"Made {decisions_made} unnecessary decisions on stable system"
    ...
    async def test_safety_validation_pipeline_comprehensive(
        ...
        from tests.services.tamiyo.test_enhanced_policy_safety import create_realistic_graph_state
        ...
        for i, decision in enumerate(test_decisions):
            if i == 1:
                ...
                service.layer_cooldowns['test_layer'] = time.time() - 10  # 10 seconds ago
...
class TestAutonomousServicePerformance:
    ...
    async def test_high_throughput_signal_processing(
        ...
        assert throughput_hz >= requirements['min_throughput_hz'], \
        ...
        assert memory_growth <= requirements['max_memory_growth_mb'], \
    ...
    async def test_sub_100ms_decision_cycle_consistency(
        ...
        assert max_latency < 200, f"Max latency {max_latency:.1f}ms exceeds 200ms hard limit"
...
class TestAutonomousServiceReliability:
    ...
    async def test_graceful_error_recovery(
        ...
        assert call_count > 3, f"Only {call_count} calls made, error recovery may not have worked"
