...
from esper.services.tamiyo.autonomous_service import (
    AutonomousTamiyoService,
    AutonomousServiceConfig,
    ServiceStatistics
)
from esper.services.tamiyo.policy import PolicyConfig
from esper.services.tamiyo.policy_trainer import ProductionTrainingConfig
from esper.services.tamiyo.reward_system import RewardConfig
...
requires_redis = pytest.mark.skipif(not REDIS_AVAILABLE, reason="Redis not available for integration tests")
...
class TestAutonomousServiceCore:
    ...
    async def test_complete_autonomous_cycle_end_to_end(
        self, 
        test_environment,
        production_service_config,
        integration_policy_config
    ):
        ...
        try:
            ...
            collection_task = asyncio.create_task(service.health_collector.start_intelligent_collection())
            ...
            if decision is not None:
                ...
                reward_components = service.reward_system.compute_reward_components(mock_outcome)
                total_reward = service.reward_system.compute_total_reward(reward_components)
                
                assert isinstance(total_reward, float)
                assert -5.0 <= total_reward <= 5.0, f"Reward {total_reward} out of expected range"
                
                logger.info(f"End-to-end cycle completed: Decision={decision.layer_name}, "
            ...
            assert decision_latency_ms < scenario.performance_requirements.max_processing_latency_ms, \
    ...
    async def test_concurrent_component_coordination(
        ...
        health_task = asyncio.create_task(service.health_collector.start_intelligent_collection())
        ...
        assert coordination_time >= 1.8, "Components stopped too early - possible deadlock"
        ...
        completed_tasks = [task for task in tasks if task.done() and not task.cancelled()]
        ...
        logger.info(f"Component coordination test completed: {coordination_time:.1f}s runtime, "
    ...
    async def test_graceful_startup_shutdown_lifecycle(
        ...
        assert service.statistics.total_decisions_made == 0, "Should start with no decisions"
        ...
        try:
            ...
            for task in service.running_tasks:
                assert task.done() or task.cancelled(), "All tasks should be completed/cancelled"
        ...
        logger.info(f"Lifecycle test completed: startup in {startup_time:.1f}s, "
    ...
    async def test_decision_safety_validation_integration(
        ...
        dangerous_scenario = ProductionScenarioFactory.create_cascading_failure_scenario()
        ...
        try:
            ...
            for attempt in range(10):
                ...
                if decision is not None:
                    ...
                    if confidence >= safety_config.min_confidence_threshold and safety_score >= 0.5:
                        # This is a safe decision
                        pass
                    else:
                        # This should have been rejected by safety validation
                        unsafe_decisions += 1
                        logger.warning(f"Unsafe decision allowed: conf={confidence:.3f}, "
            ...
            total_rejections = (final_safety_rejections - initial_safety_rejections +
                              final_confidence_rejections - initial_confidence_rejections)
            ...
            logger.info(f"Safety validation test: {total_rejections}/{decisions_attempted} "
    ...
    async def test_error_recovery_and_resilience(
        ...
        error_count = 0
        original_consume = mock_oona.consume.side_effect
        ...
        try:
            ...
            # Start health monitoring with error injection
            health_task = asyncio.create_task(service._health_monitoring_loop())
            ...
            assert stats["service"]["runtime_hours"] >= 0, "Runtime tracking failed during errors"
            
            logger.info(f"Error recovery test: survived {error_count} connection failures")
...
class TestAutonomousServicePerformance:
    ...
    # @requires_redis
    @pytest.mark.asyncio
    async def test_100ms_decision_cycle_consistency(
        self,
        performance_config
    ):
        ...
        scenario = ProductionScenarioFactory.create_high_throughput_scenario(signal_count=1000)
        
        # Pre-populate health signals
        health_signals = scenario.health_signals[:100]  # Use subset for performance test
        ...
        for i in range(50):  # Test 50 decision cycles
            ...
            graph_state = service.graph_builder.build_model_graph(health_signals[i:i+20])
        ...
        assert avg_latency < 50.0, f"Average latency {avg_latency:.1f}ms exceeds 50ms target"
        assert p95_latency < requirements.max_processing_latency_ms, \
            f"P95 latency {p95_latency:.1f}ms exceeds {requirements.max_processing_latency_ms}ms requirement"
        assert p99_latency < 150.0, f"P99 latency {p99_latency:.1f}ms exceeds 150ms limit"
        assert max_latency < 200.0, f"Max latency {max_latency:.1f}ms exceeds 200ms hard limit"
        ...
        logger.info(f"Decision cycle performance: Avg={avg_latency:.1f}ms, "
    ...
    async def test_memory_usage_stability_under_load(self):
        ...
        for batch in range(100):  # 100 batches
            ...
            if batch % 20 == 0:
                ...
                assert memory_growth < 100, f"Memory growth {memory_growth}MB too high at batch {batch}"
        ...
        assert total_memory_growth < max_allowed_growth, \
        ...
        logger.info(f"Memory stability test: {total_memory_growth}MB growth over 2000 signals, "
