...
class TestSimplifiedPolicySafety:
        ...
        for key, value in output.items():
            if isinstance(value, torch.Tensor):
                ...
                for subkey, subtensor in value.items():
                    assert isinstance(subtensor, torch.Tensor), f"{key}.{subkey} should be tensor"
                    assert torch.isfinite(subtensor).all(), f"{key}.{subkey} should have finite values"
            else:
    ...
    def test_safety_regularizer_directly(self, safety_policy):
        ...
        assert (0 <= safety_score).all() and (safety_score <= 1).all(), "Safety score should be in [0,1]"
    ...
    def test_uncertainty_quantification(self, safety_policy):
        ...
        if not hasattr(safety_policy, 'uncertainty_module') or safety_policy.uncertainty_module is None:
    ...
    def test_policy_with_different_scenarios(self, safety_policy):
        ...
        for i, signal in enumerate(signals):
            ...
            node_features[i, 5:] = torch.randn(safety_policy.config.node_feature_dim - 5) * 0.1
        ...
        unhealthy_features = torch.zeros(num_nodes, safety_policy.config.node_feature_dim)
        for i, signal in enumerate(unhealthy_signals):
            ...
            unhealthy_features[i, 5:] = torch.randn(safety_policy.config.node_feature_dim - 5) * 0.1
        ...
        for key in ["adaptation_prob", "safety_score"]:
            ...
            logger.info(f"Policy comparison - {key}: healthy={healthy_val:.3f}, unhealthy={unhealthy_val:.3f}")
    ...
    def test_policy_consistency(self, safety_policy):
        ...
        for i, output in enumerate(outputs[1:], 1):
            for key in base_output:
                if isinstance(base_output[key], torch.Tensor) and isinstance(output[key], torch.Tensor):
                    ...
                    assert diff < tolerance, f"Output {key} inconsistent between runs: max_diff={diff}"
                elif isinstance(base_output[key], dict) and isinstance(output[key], dict):
                    # Handle nested dictionaries
                    for subkey in base_output[key]:
                        if isinstance(base_output[key][subkey], torch.Tensor):
                            diff = torch.abs(output[key][subkey] - base_output[key][subkey]).max().item()
                            tolerance = 0.05
                            assert diff < tolerance, f"Output {key}.{subkey} inconsistent: max_diff={diff}"
