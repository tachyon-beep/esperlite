...
from esper.services.tamiyo.health_collector import (
...
def create_test_message(signal: HealthSignal, topic: TopicNames = TopicNames.TELEMETRY_SEED_HEALTH) -> OonaMessage:
...
class TestSignalFilterEngineCore:
    ...
    def test_filter_engine_throttling_with_production_load(self):
        ...
        assert 0.05 <= throttle_rate <= 0.30, f"Throttle rate {throttle_rate:.1%} outside expected range"
        
        logger.info(f"Throttled {total_signals} signals to {processed_count} ({throttle_rate:.1%})")
...
class TestHealthSignalBufferCore:
    ...
    async def test_buffer_priority_queueing_with_real_signals(self):
        ...
        for scenario in scenarios:
            for signal in scenario.health_signals[:50]:  # 50 signals each
                priority = 0.9 if scenario.scenario_type == ScenarioType.UNHEALTHY_SYSTEM else 0.3
        ...
        assert len(retrieved) >= 20, f"Expected at least 20 signals, got {len(retrieved)}"
        assert len(retrieved) <= 100, f"Expected at most 100 signals, got {len(retrieved)}"
        
        # Count high priority signals in retrieved set
        high_priority_count = sum(
            1 for signal, priority in added_signals
            if priority > 0.7 and any(
        ...
        assert retention_rate >= 0.8, f"High priority retention rate {retention_rate:.1%} too low"
    ...
    async def test_buffer_concurrent_access_performance(self):
        ...
        async def add_batch(batch_start: int, batch_size: int):
            for i in range(batch_start, min(batch_start + batch_size, len(signals_batch))):
                signal, priority = signals_batch[i]
                await buffer.add_with_priority(signal, priority)
        
        # Run 10 concurrent add operations
        await asyncio.gather(*[
            add_batch(i * 500, 500) for i in range(10)
        ])
        ...
        read_tasks = [
            buffer.get_recent_signals(1000) for _ in range(5)
        ]
        ...
        assert add_throughput > 10000, f"Add throughput {add_throughput:.0f} signals/sec too low"
        ...
        logger.info(f"Buffer performance: {add_throughput:.0f} adds/sec, {read_time*1000:.1f}ms read time")
    ...
    async def test_buffer_memory_efficiency_under_load(self):
        ...
        for wave in range(10):
            ...
            assert current_size <= buffer.max_size * 1.1  # Allow 10% overflow for priority queue
        ...
        logger.info(f"Buffer size: initial={initial_buffer_size}, final={final_size}, max={buffer.max_size}")
...
class TestErrorRecoveryIntegration:
    ...
    def test_error_recovery_signal_conversion_with_real_errors(self):
        ...
        logger.info(f"Converted error events: success_health={success_signal.health_score:.2f}, "
...
class TestProductionHealthCollectorCore:
    ...
    async def test_intelligent_signal_filtering_accuracy(self, mock_oona_client):
        ...
        collector = ProductionHealthCollector(
            oona_client=mock_oona_client,
            buffer_size=1000,
            processing_batch_size=100
        )
        
        # Test the core components directly instead of through message parsing
        unhealthy_scenario = ProductionScenarioFactory.create_unhealthy_system_scenario()
        ...
        assert unhealthy_rate >= stable_rate, "Unhealthy signals should have higher processing rate"
        assert stats["signals_processed"] > 0, "Should have processed some signals"
        
        logger.info(f"Filter results: unhealthy_rate={unhealthy_rate:.1%}, stable_rate={stable_rate:.1%}, "
    ...
    async def test_buffer_management_under_realistic_load(self, mock_oona_client):
        ...
        high_throughput_scenario = ProductionScenarioFactory.create_high_throughput_scenario(
        ...
        assert actual_throughput >= 1000, f"Throughput {actual_throughput:.0f} signals/sec too low"
        ...
        assert len(recent_signals) == 1000, "Buffer retrieval failed"
        
        logger.info(f"Load test results: {actual_throughput:.0f} signals/sec throughput, "
    ...
    async def test_oona_integration_reliability(self, mock_oona_client):
        ...
        logger.info(f"Integration test completed: {stats['signals_processed']} signals processed, "
    ...
    def _simulate_redis_streams_behavior(self):
        ...
        async def mock_consume(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                # Normal message flow
                test_signal = ProductionHealthSignalFactory.create_healthy_signal(call_count, epoch=100)
...
class TestHealthCollectorPerformance:
    ...
    async def test_sub_50ms_processing_latency(self, real_oona_client_mock):
        ...
        scenario = ProductionScenarioFactory.get_scenario_by_type(ScenarioType.HIGH_THROUGHPUT)
        ...
        assert p99_latency < performance_requirements.p95_latency_ms, \
            f"P99 latency {p99_latency:.2f}ms exceeds requirement {performance_requirements.p95_latency_ms}ms"
        
        assert p95_latency < performance_requirements.p95_latency_ms, \
        ...
        assert report["requirements_met"], f"Performance requirements not met: {report['violations']}"
        
        logger.info(f"Processing latency results: Avg={avg_latency:.2f}ms, "
    ...
    async def test_10k_signals_per_second_throughput(self, real_oona_client_mock):
        ...
        scenario = ProductionScenarioFactory.create_high_throughput_scenario(signal_count=15000)
        ...
        assert actual_throughput >= min_throughput, \
            f"Throughput {actual_throughput:.0f} Hz below requirement {min_throughput} Hz"
        ...
        assert stats["error_count"] == 0, "Processing errors occurred during throughput test"
        
        logger.info(f"Throughput test results: {actual_throughput:.0f} signals/sec "
                   f"(requirement: {min_throughput:.0f} signals/sec)")

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_memory_usage_stability_24_hour_simulation(self, real_oona_client_mock):
        ...
        max_allowed_growth = scenario.performance_requirements.max_memory_growth_mb if hasattr(scenario, 'performance_requirements') else 100
        
        assert memory_growth_mb <= max_allowed_growth, \
            f"Memory growth {memory_growth_mb:.1f}MB exceeds limit {max_allowed_growth}MB"
        
        # Buffer should stay within reasonable bounds
        assert max_memory <= collector.signal_buffer.max_size * 1.2, "Memory usage exceeded buffer limits"
        
        # Processing should continue working
        final_stats = collector.get_collection_stats()
        assert final_stats["error_count"] < processed_total * 0.01, "Too many processing errors over time"
        
        logger.info(f"24-hour simulation: {processed_total} signals processed, "
