...
class CoverageAnalyzer:
    ...
    def run_coverage_analysis(self, test_paths: Optional[List[str]] = None) -> Dict[str, float]:
        """
    ...
    def generate_coverage_report(self) -> str:
        ...
        report_lines = [
            "=" * 80,
            "ESPER PLATFORM TEST COVERAGE REPORT",
            "=" * 80,
            ""
        ]
        ...
        overall_coverage = sum(all_coverages) / len(all_coverages) if all_coverages else 0
        ...
        low_coverage_modules = [
            (name, cov) for name, cov in self.coverage_data.items()
        ...
        report_lines.extend([
            ...
            "Coverage Goals:",
            "-" * 20,
            f"• Core execution modules: >95% (critical for reliability)",
            f"• Service modules: >90% (important for integration)",
            f"• Utility modules: >85% (good for maintainability)",
            f"• Contract modules: >80% (basic validation)",
            ""
        ])
    ...
    def check_coverage_gates(self) -> Tuple[bool, List[str]]:
        ...
        if overall_coverage < self.min_coverage:
            failures.append(f"Overall coverage {overall_coverage:.1f}% below minimum {self.min_coverage:.1f}%")
        ...
        for module in critical_modules:
            ...
            if coverage < 95.0:  # Higher standard for critical modules
                failures.append(f"Critical module {module} coverage {coverage:.1f}% below 95% requirement")
        ...
        if zero_coverage_modules:
            failures.append(f"Modules with zero coverage: {', '.join(zero_coverage_modules)}")
    ...
    def check_test_file_structure() -> Tuple[bool, List[str]]:
        ...
        for test_file in test_dir.rglob("test_*.py"):
            ...
            if rel_path.name != "test_infrastructure.py":  # Skip infrastructure tests
                source_path = Path("src") / "esper" / rel_path.parent / rel_path.name.replace("test_", "").replace(".py", ".py")
                if not source_path.exists() and "test_" in rel_path.name:
                    # Check for alternative naming patterns
                    alt_source = Path("src") / "esper" / rel_path.parent / rel_path.name.replace("test_", "")
                    if not alt_source.exists():
                        issues.append(f"Test file {test_file} has no corresponding source file")
                    ...
                    if line.strip().startswith("class Test") and ":" in line:
                        ...
                        for j in range(i + 1, min(i + 5, len(lines))):
                            ...
                            if next_line and not next_line.startswith('"""') and not next_line.startswith("'''"):
                                issues.append(f"Test class in {test_file} line {i+1} missing docstring")
                                break
                ...
                try:
                    ...
                    for method in required_test_methods:
                        if method not in content:
                            issues.append(f"Performance test {req_test} missing {method}")
...
class TestCoverageIntegration:
    ...
    def test_test_file_structure_check(self):
        ...
        critical_missing = [issue for issue in issues if "Missing required test directory" in issue]
        assert len(critical_missing) == 0, f"Critical test directories missing: {critical_missing}"
